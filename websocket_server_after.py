...
class OptimizedRealTimeDetector:
    def __init__(self):
        ...
        self.client_sequence_counters = {}  # Sequence counters for organizing frames
    ...
    def _apply_mediapipe_mask_and_grayscale(self, image_rgb: np.ndarray) -> tuple:
        """Apply MediaPipe masking and convert to grayscale with FastAPI-level processing"""
        ...
        results = self.hands_detector.process(enhanced_image)
        ...
        if not results.multi_hand_landmarks:
            ...
            results = self.hands_detector.process(bright_image)
            ...
            if not results.multi_hand_landmarks:
                ...
                results = self.hands_detector.process(sharp_image)
                ...
                if len(landmark_points) >= 3:
                    try:
                        ...
                        for point in landmark_points:
                            cv2.circle(mask, tuple(point), 12, 255, -1)  # Increased radius
                else:
                    for point in landmark_points:
                        cv2.circle(mask, tuple(point), 12, 255, -1)  # Increased radius
        ...
        if np.any(mask > 0):  # Valid hand detected
            ...
            logger.warning("No hands detected, trying fallback skin color detection")
            # Try to create a basic mask based on skin color as last resort
            fallback_mask = self._create_fallback_hand_mask(image_rgb)
            if np.any(fallback_mask > 0):
                fallback_gray = cv2.bitwise_and(gray_source, gray_source, mask=fallback_mask)
                return fallback_gray, fallback_mask
            return np.zeros_like(gray_source), mask
    
    def _create_fallback_hand_mask(self, image_rgb: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if contours:
                ...
                cv2.fillPoly(fallback_mask, [largest_contour], 255)
                
                # Smooth the mask
                fallback_mask = cv2.medianBlur(fallback_mask, 19)
            ...
            logger.warning(f"Fallback skin detection failed: {e}")
            return np.zeros(image_rgb.shape[:2], dtype=np.uint8)
    ...
    def process_frame(self, client_id: str, frame_data: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Apply MediaPipe masking with enhanced processing
    ...
    def cleanup_client(self, client_id: str):
        ...
        if client_id in self.client_sessions:
            del self.client_sessions[client_id]        # Clean up debug directories tracking
        ...
        if client_id in self.client_sequence_counters:
            del self.client_sequence_counters[client_id]
    ...
    def _setup_client_debug_dir(self, client_id: str) -> str:
        ...
        if client_id not in self.client_debug_dirs:
            ...
            self.client_sequence_counters[client_id] = 0
    ...
    def _save_websocket_frame(self, client_id: str, frame: np.ndarray, processed_frame: Optional[np.ndarray] = None) -> None:
        """Save a frame from WebSocket to debug directory with FastAPI-like structure"""
        try:
            ...
            # Create sequence folder when we start a new sequence (every 16 frames)
            sequence_frame_idx = (frame_num - 1) % config.SEQUENCE_LENGTH
            if sequence_frame_idx == 0:
                self.client_sequence_counters[client_id] += 1
            
            sequence_num = self.client_sequence_counters[client_id]
            sequence_dir = os.path.join(debug_dir, f"sequence_{sequence_num:03d}")
            os.makedirs(sequence_dir, exist_ok=True)
            
            # Save input frame (original frame like FastAPI)
            input_frame_filename = f"frame_{sequence_frame_idx:02d}_input.png"
            input_frame_path = os.path.join(sequence_dir, input_frame_filename)
            cv2.imwrite(input_frame_path, frame)
            
            # Save masked frame if provided and has content
            if processed_frame is not None and np.any(processed_frame > 0):
                masked_frame_filename = f"frame_{sequence_frame_idx:02d}_masked.png"
                masked_frame_path = os.path.join(sequence_dir, masked_frame_filename)
                ...
                if frame_num <= 5:  # Log first 5 frames for debugging
                    ...
                    logger.info(f"Sequence {sequence_num}, Frame {sequence_frame_idx} - Original: {original_size}, Masked: {masked_size}, Non-zero pixels: {masked_nonzero}")
            else:
                if frame_num <= 5:
                    logger.warning(f"Sequence {sequence_num}, Frame {sequence_frame_idx}: No valid masked frame (empty or None) for {client_id}")
            
            # Create sequence info when sequence is complete
            if sequence_frame_idx == config.SEQUENCE_LENGTH - 1:
                self._create_sequence_info(client_id, sequence_dir, sequence_num)
            
            # Log every 10th frame to avoid spam
            if frame_num % 10 == 0:
                logger.info(f"Saved frame {frame_num} (sequence {sequence_num}, frame {sequence_frame_idx}) for {client_id}")
    ...
    def _create_sequence_info(self, client_id: str, sequence_dir: str, sequence_num: int) -> None:
        """Create sequence info file similar to FastAPI's prediction_info.txt"""
        try:
            sequence_info_path = os.path.join(sequence_dir, "sequence_info.txt")
            
            # Get recent motion history for this sequence
            motion_history = list(self.client_motion_histories.get(client_id, []))
            recent_motion = motion_history[-config.SEQUENCE_LENGTH:] if len(motion_history) >= config.SEQUENCE_LENGTH else motion_history
            
            # Calculate motion statistics
            avg_motion = sum(recent_motion) / len(recent_motion) if recent_motion else 0
            max_motion = max(recent_motion) if recent_motion else 0
            min_motion = min(recent_motion) if recent_motion else 0
            
            with open(sequence_info_path, "w") as f:
                f.write(f"WebSocket Sequence Info\n")
                f.write(f"======================\n")
                f.write(f"Client ID: {client_id}\n")
                f.write(f"Sequence Number: {sequence_num}\n")
                f.write(f"Frames in Sequence: {config.SEQUENCE_LENGTH}\n")
                f.write(f"Motion Threshold: {self.motion_threshold}\n")
                f.write(f"Confidence Threshold: {self.confidence_threshold}\n\n")
                
                f.write(f"Motion Analysis for this Sequence:\n")
                f.write(f"----------------------------------\n")
                ...
                if recent_motion:
                    f.write(f"Frame-by-Frame Motion:\n")
                    f.write(f"----------------------\n")
                    for i, motion in enumerate(recent_motion):
                        f.write(f"Frame {i:02d}: {motion:.6f} {'✓' if motion > self.motion_threshold else '✗'}\n")
            
            logger.info(f"Created sequence info for sequence {sequence_num} of {client_id}")
            
        except Exception as e:
            logger.error(f"Error creating sequence info: {e}")
    ...
    def intelligent_frame_skip(self, client_id: str, frame_data: str) -> bool:
        """Determine if frame should be skipped based on various factors"""
        # Simple implementation - could be enhanced with actual frame analysis
        return False
        
    async def handle_reconnection(self, client_id: str, websocket):
        """Handle client reconnection"""
        # Restore client session if exists
        if client_id in self.client_sessions:
            logger.info(f"Restoring session for {client_id}")
        else:
            logger.info(f"New session for {client_id}")
            self.client_sessions[client_id] = time.time()
        ...
        async for message in websocket:
            try:
                ...
                if message_type == "frame":
                    ...
                    detector.motion_threshold = max(0.001, min(0.01, detector.motion_threshold))
                    response = {
...
async def main():
    ...
    # Configure WebSocket server with appropriate limits for image streaming
